#include "rock_detectionoutput.h"
#include <algorithm>
#include <math.h>
#include <stdio.h>
...
RockDetectionOutput::RockDetectionOutput(DetectionOutputParam &detection_param)
{
    num_class = detection_param.num_class;
	nms_threshold = detection_param.nms_threshold;
	nms_top_k = detection_param.nms_top_k;
	keep_top_k = detection_param.keep_top_k;
	confidence_threshold = detection_param.confidence_threshold;
}
...
int RockDetectionOutput::forward(const std::vector<rock::Mat>& bottom_blobs, rock::Mat& top_blobs)
{
    const rock::Mat& location   = bottom_blobs[0];
    const rock::Mat& confidence = bottom_blobs[1];
    const rock::Mat& priorbox   = bottom_blobs[2];

    /*
    printf("locaktion offset shape = [%d x %d x %d]\n",location.c,location.h,location.w);
    printf("confidence shape = [%d x %d x %d]\n",confidence.c,confidence.h,confidence.w);
    printf("priorbox shape = [%d x %d x %d]\n",priorbox.c,priorbox.h,priorbox.w);
    */
    ...
    for (int i = 1; i < num_class; i++)
    {
        ...
        for (int j = 0; j < num_prior; j++)
        {
            ...
            {
                // 计算通过阈值的框坐标
                const float* loc = location_ptr + j * 4;
                ...
                BBoxRect c = { bbox_0, bbox_1, bbox_2, bbox_3, i };
